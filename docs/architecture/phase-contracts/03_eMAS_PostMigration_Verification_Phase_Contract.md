...
**Canonical references:** Enterprise Requirements v3.1 §14.3; Solution Architecture v1.0; Runtime JSON Contract v1.2; Runtime JSON Schema 1.0.0
...
- optional portable WPF that invokes the same Post-Migration entry script.
...
Console output must show baseline loading, MigrationSummary reading, comparison stages, report generation and completion. The log records comparison counts, skipped/unsupported evidence, reader/version decisions, discrepancy totals, exception treatment and generated file paths.

## 11. Failure behavior

Stop before comparison when configuration, template or required baseline integrity fails.
...
- CLI and optional WPF use the same entry script and engine;
