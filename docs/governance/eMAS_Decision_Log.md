...
| DEC-2026-016 | MVP Requirement Catalogue review | 2026-09-13 | Approved MVP design | Use one atomic, testable requirement per `06_Requirement_Catalogue` row; support assessment-module and cross-cutting workbook/transformer/runtime/reporting/logging ownership; separate lifecycle stage, phase scope and Global/ModuleDriven/ScenarioSpecific applicability; remove direct RuleId storage and derive one-to-many implementation traceability from rule-sheet `RequirementId` references; separate requirement, implementation and verification status; require 24 `REQ-*` families and source-statement disposition rather than an arbitrary row count; serialize applicable runtime requirements as structured `requirements[]` objects and validate bidirectional requirement/implementation coverage. | Product Owner | Enterprise requirements, Mapping Workbook/JSON MVP requirements, Requirement Catalogue, Final Config Master, JSON contract, validation and acceptance tests |
| DEC-2026-017 | MVP Fields/Evidence and Value Lists review | 2026-09-13 | Approved MVP design | Define `07_Fields_Evidence` as the canonical semantic dictionary for reusable fields and `22_Value_Lists` as the controlled machine-code/label authority. Store one field definition per stable code with type/cardinality/domain, origin, producer, normalization, provenance, datatype-appropriate operator list, phase availability, baseline/reconciliation role and safe report/log handling. Keep exact XML/source extraction locations in profiles and rule/mapping sheets; keep actual customer observations out of reusable configuration; prohibit assumed evidence; keep value, evidence state, evaluation status, RAG/severity and confidence independent; define `MISSING` as ConfirmedAbsent only; include fields and value lists in scenario JSON through deterministic transitive dependency closure. DMS fields do not authorize DMS-to-DMS scope. | Product Owner | Enterprise requirements, Mapping Workbook/JSON MVP requirements, Fields/Evidence, Value Lists, Final Config Master, JSON contract, validation and acceptance tests |
...
For the `requirements/mvp-workbook-json` branch, DEC-2026-013 through DEC-2026-017 are consolidated into Enterprise Requirements v5.0 and Mapping Workbook and Scenario JSON MVP Requirements v4.5. These approvals close the base-scenario, questionnaire, scenario-derivation, assessment-module, scenario-module-map, Requirement Catalogue, Fields/Evidence and Value Lists design reviews only. They do not complete workbook construction, JSON transformation, schema synchronization, PowerShell, templates, tests or release controls.
