...
| CFG-JSON | [04 — Runtime JSON Contract](04_eMAS_Runtime_JSON_Contract.md) | v1.1 Approved design baseline | Runtime structure, versioning, validation and compatibility |
...
Where the content catalogue or earlier requirement prose uses a shorthand repeating field such as `SupportedPhases`, `AllowedOperators`, `DriverCodeOrOverallModel` or `TriggerFieldOrFinding`, the dedicated link entity or typed discriminator defined by CFG-REL and CFG-DICT controls the normalized physical model. The shorthand may be retained only as a business-facing view or generated display and must not be implemented as comma-separated or ambiguous free text.
